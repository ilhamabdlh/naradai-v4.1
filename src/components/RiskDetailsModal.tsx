import { X, ThumbsDown, Twitter, Instagram, Facebook, MessageSquare } from "lucide-react";
import type { RiskItem } from "@/lib/dashboard-content-types";
...
export function RiskDetailsModal({ risk, onClose }: RiskDetailsModalProps) {
  const sourceList = risk.sourceContent?.length
    ? risk.sourceContent
    : [];
      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-700">
              Supporting contents: {risk.supportingContents}
            </span>
          </div>
          <div>
            <div className="text-sm text-slate-700 font-medium mb-2">Impact Assessment</div>
            <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
              <p className="text-slate-900">{risk.impact}</p>
            </div>
          </div>
          <div>
            <div className="text-sm text-slate-700 font-medium mb-2">Mitigation Strategy</div>
            <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
              ...
              {sourceList.map((post) => (
                <div key={post.id} className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="text-xs font-semibold px-2 py-1 rounded-lg bg-red-100 text-red-700">
